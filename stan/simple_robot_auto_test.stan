...
model {
	matrix[2,N] tau_hat;
  row_vector[N] x0 = 9.81*sin(q[1,:]);
	row_vector[N] x1 = 9.81*cos(q[1,:]);
	row_vector[N] x2 = sin(q[2,:]);
	row_vector[N] x3 = -((dq[1,:] + dq[2,:]).*(dq[1,:] + dq[2,:]));
	row_vector[N] x4 = ddq[1,:] + ddq[2,:];
	row_vector[N] x5 = a1*ddq[1,:] + x1;
	row_vector[N] x6 = -a1*((dq[1,:]).*(dq[1,:])) + x0;
	row_vector[N] x7 = cos(q[2,:]);
	row_vector[N] x8 = x2.*x5 + x6.*x7;
	row_vector[N] x9 = -x2.*x6 + x5.*x7;
	row_vector[N] x10 = params[17]*x4 + params[18]*x9 - params[19]*x8;
//
	tau_hat[1,:] = a1*(x2.*(params[18]*x3 - params[19]*x4 + params[21]*x8) + x7.*(params[18]*x4 + params[19]*x3 + params[21]*x9)) + ddq[1,:]*params[6] + dq[1,:]*params[11] + params[7]*x1 - params[8]*x0 + x10;
